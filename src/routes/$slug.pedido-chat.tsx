...
/**
 * Versão experimental do formulário de pedido, em formato de conversa —
 * rota separada de `/$slug/pedido` pra comparar qual das duas os clientes
 * preferem sem mexer no fluxo que já está em produção. Mesma lógica de
 * negócio e mesmas server functions do formulário original; só a
 * apresentação muda (uma pergunta de cada vez, respostas prontas quando dá
 * pra prever a resposta, texto livre quando não dá).
 */
export const Route = createFileRoute("/$slug/pedido-chat")({
  loader: async ({ params }) => {
